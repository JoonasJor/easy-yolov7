...
def backWards():
    ...
    for input in inputs:
        ...
        print(neginputs)
        swarm.tellos[0].send_rc_control(neginputs[0], neginputs[1], neginputs[2], neginputs[3])
        sleep(interval)
...
def drawPoints():
    for d in range(len(swarm.tellos)):
        ...
        cv2.putText(img, f'({(x[d] - 500) / 100}, {(y[d] - 500) / 100})m', (x[d] + 10, y[d] + 30),
                    cv2.FONT_HERSHEY_PLAIN, 1, colors[d], 1)
